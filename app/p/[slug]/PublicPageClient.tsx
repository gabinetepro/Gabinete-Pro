...
import { Send, CheckCircle2, RefreshCw, AlertCircle, MapPin, ExternalLink } from "lucide-react";
...
interface FormState { nome: string; telefone: string; mensagem: string; }
...
function ConfirmationScreen({ protocolo, nomeEnviou, onNovaMensagem }: {
  protocolo: string; nomeEnviou: string; onNovaMensagem: () => void;
}) {
  return (
    <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
      ...
      <p className="text-slate-500 text-sm mb-6 max-w-xs leading-relaxed">
        Obrigado, <strong className="text-slate-700">{nomeEnviou}</strong>! Em breve nossa equipe entrará em contato.
      </p>
...
export default function PublicPageClient({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSending(false);
    if (insertErr) { setError("Ocorreu um erro ao enviar. Por favor, tente novamente."); return; }
    setProtocolo(prot);
  }
        ...
        <div className="max-w-md mx-auto relative">
          ...
          <div className="flex flex-col items-center text-center gap-4">
            ...
            <div>
              ...
              {profile.cargo && <p className="text-blue-200 font-semibold text-sm mt-1">{profile.cargo}</p>}
              <div className="flex items-center justify-center flex-wrap gap-2 mt-3">
                {profile.partido && (
                  <span className="text-xs bg-white/15 text-white px-3 py-1 rounded-full font-semibold backdrop-blur-sm">{profile.partido}</span>
                )}
                {localidade && (
      ...
      <div className="max-w-md mx-auto px-4 -mt-8 pb-12 relative z-10 space-y-5">
        ...
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100">
          <div className="px-6 py-5 border-b border-slate-100">
            <h2 className="text-slate-800 font-bold text-lg">{profile.nome_gabinete ?? "Fale com o gabinete"}</h2>
            <p className="text-slate-400 text-sm mt-0.5">Sua mensagem será respondida em breve pela equipe.</p>
          </div>
          {protocolo ? (
            <ConfirmationScreen
              protocolo={protocolo}
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input className={inputCls} placeholder="Seu nome completo" value={form.nome} onChange={(e) => setF("nome", e.target.value)} />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wide">Telefone</label>
                <input className={inputCls} placeholder="(00) 00000-0000" value={form.telefone} onChange={(e) => setF("telefone", maskPhone(e.target.value))} />
              </div>
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={`text-xs ${form.mensagem.length > 480 ? "text-red-500" : "text-slate-300"}`}>{form.mensagem.length}/500</span>
                </div>
                <textarea
              ...
              >
                {sending ? <><RefreshCw size={16} className="animate-spin" /> Enviando…</> : <><Send size={16} /> Enviar mensagem</>}
              </button>
            </form>
          )}
        </div>
                  ...
                  <div key={d.id} className="px-6 py-4">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        {d.categoria && <p className="text-xs text-slate-400 mt-0.5">{d.categoria}</p>}
                      </div>
                      <span className={`shrink-0 text-[11px] font-semibold px-2.5 py-1 rounded-full border ${st.cls}`}>{st.label}</span>
                    </div>
                  </div>
                );
              })}
        ...
        <p className="text-center text-xs text-slate-400 pt-2">
          Powered by{" "}
          <a href="https://gabinete-pro.vercel.app" target="_blank" rel="noopener noreferrer" className="text-blue-500 font-semibold inline-flex items-center gap-1 hover:text-blue-600 transition-colors">
            Gabinete Pro<ExternalLink size={10} />
          </a>
        </p>
      </div>
    </div>
